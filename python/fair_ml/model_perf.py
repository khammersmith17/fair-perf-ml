...
def linear_regression_analysis(
    ...
    res: dict = model_performance_regression(
        y_true=y_true,
        y_pred=y_pred
    )
...
def binary_classification_analysis(
    ...
    y_true: NDArray = check_and_convert_type(y_true) #pyright: ignore
    y_pred: NDArray = check_and_convert_type(y_pred) #pyright: ignore
    res: dict = model_performance_classification(
        y_true=y_true,
        y_pred=y_pred
    )
    return ModelPerformance(
...
def runtime_check_full(latest: dict, baseline: dict, threshold: Optional[float] = None) -> dict:
...
def partial_runtime_check(latest: dict, baseline: dict, threshold: Optional[float], metrics: List[str]) -> dict:
    model_type = baseline.get("modelType")
    if model_type is None:
        raise InvalidMetricsBody("Invalid metrics body")
