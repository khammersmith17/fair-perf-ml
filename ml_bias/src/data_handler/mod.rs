...
pub fn determine_type(py: Python<'_>, array: &Bound<'_, PyUntypedArray>) -> PassedType {
    ...
    if element_type.is_equiv_to(&dtype_bound::<f64>(py))
        | element_type.is_equiv_to(&dtype_bound::<f64>(py))
    {
        PassedType::Float
...
pub fn apply_label<'py>(
    ...
    let labeled_array: Vec<i16> = match pred_type {
        PassedType::String => {
            let data_vec: Vec<String> = iter.map(|item| item.unwrap().extract().unwrap()).collect();
            let data_label: String = label.extract().expect(
                "prediction_segmentation must be str when prediction_array contains type str",
            );
        ...
        PassedType::Float => {
            let data_vec: Vec<f64> = iter
                ...
                .collect::<Vec<f64>>();
            let data_label: f64 = label.extract().expect(
                "prediction_segmentation must be float when prediction_array contains type float",
            );
        ...
        PassedType::Integer => {
            ...
            let data_set: std::collections::HashSet<i32> = data_vec
                ...
                .map(|value| *value as i32)
                .collect::<std::collections::HashSet<_>>();
            let data_label = label.extract().expect(
                "prediction_label_value_or_threshold must be an int when the array type is an int",
            );
            if data_set.len() == 2 {
                apply_label_discrete(data_vec, data_label)
            } else {
...
pub fn perform_segmentation_model_bias(
    ...
    (facet_a_trues, facet_a_preds, facet_d_trues, facet_d_preds)
}
